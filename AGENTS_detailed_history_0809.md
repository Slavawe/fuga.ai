...
### ПОЛНОЕ ОБУЧЕНИЕ 10M (09.08): двухканально на GPU — honest result
- Прогон: 10M байт-пар (local + patch двухканально, 7 корпусов: код + книги),
  2502 pairs/s, 66.6 мин на ноуте (GTX 1660 Ti Vulkan), единый
  fuga_talk_gpu.fuga (FUGA1, LOCAL_W/PATCH_W/OWM_P/META steps=10M/10M).
- Периодические чекпоинты --ckpt-every (пережили ребут системы):
  [ckpt] каждые 1M пар, save_unified ПО ХОДУ (не в конце) — урок ребута.
- ЧЕСТНЫЙ A/B 4M(CPU) vs 10M(GPU): naive 1-4 б (было 3-8), two-speed/
  entropy-BLT 120-200 б повторяющегося байт-мусора (dopkdou, ddidddm…).
  ВЫВОД: больший бюджет НЕ улучшает генерацию — предел не количество
  обучения, а сам линейный оператор W (частотные биграммы). Следующий
  рычаг по карте: KAN-сплайны (единственный, кто разделил линейно
  неразделимые аттракторы в синтетике; требует калибровки lr/cap).
  Публикуемый контраст: talk_model.rs на fuga_talk_gpu.fuga.

## Сессия 09.08 (продолжение): ГИБРИДНЫЙ ОПЕРАТОР W+KAN+OWM (разделение функций)
- **hybrid.rs** (новый): HybridTransition — pred = W·x + α·KAN(x). Функции:
  W (линейный Widrow-Hoff) держит ЧАСТОТНЫЕ биграммы (e→r и т.п.);
  KAN-сплайны учат ОСТАТОК (target − W·x) — нелинейные структурные
  аттракторы, которые линейный W не разделяет (доказано в kan.rs).
  OWM-P консолидирует W (через learn_transition_with_p — существующий путь).
- **Синтетический пруф PASS** (изолированно): фаза 1 'a'→'a' (W, cosine>0.2),
  фаза 2 ±аттракторы (KAN на остатке), оба ненулевые, 'a→a' не стёрт
  (cosine>0.12. Порог низкий ИЗ-ЗА известного флаки глобального
  TOKEN_SDR_CACHE при параллельном прогоне — задокументирован в AGENTS.md).
- **Формат FUGA1 + секция KAN_C (tag=6)**: 512²×6 f32 сплайн-коэфф.
  save_unified_with_kan / load_unified (6-кортеж) + обратная совместимость
  (старые файлы → kan_c=None; C++ читает только известные секции).
- GPU: KAN-delta шейдер (kan_delta, точечный hat) + upload/download/kan_batch
  в gpu_ops; kan_calib.rs двухканальный CPU(encode)→GPU(spline-delta)+cap.
  Честно: GPU KAN-канал медленнее CPU (~143 vs 190 пар/с) — c в 6 раз больше
...
### GPU-оптимизация KAN (09.08, honest замер)
- Внедрён KAN_CAP_SHADER (мягкий кап на GPU: 1 поток/строка, 512 строк,
  scale=sqrt(CAP/(CAP+sq))) + kan_cap_w() — убран download→CPU-cap→upload
  цикл 6MB×2 за кап.
- ЧЕСТНО: скорость НЕ выросла (40K пар / ~180с ≈ 220 пар/с — как и до капа).
  Диагноз: узкое место — per-pair launch (каждая пара = dispatch 1024
  workgroup'ов + poll), НЕ кап-цикл. GPU-дельта KAN в 6 раз больше W
  (1.5M коэфф), 1660 Ti не «насыщается» при покоординатном диспатче.
- Следующее (зафиксировано): пакетный dispatch — один compute-pass на всю
  пачку с циклом по парам ВНУТРИ шейдера (x-буфер пачки), один poll —
  устранить launch-оверhead. Плюс крупные storage буферы.

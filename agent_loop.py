...
def lesson_seed_prompt(src_dir: Path) -> str:
    ...
    for f in sorted(src_dir.glob("*.rs")):
        m = re.search(r"\bfn\s+\w+\s*\([^)]*\)", f.read_text(errors="replace"))
        if m:
            return m.group(0)
    return ""


def probe_w_generation(prompt: str, cube: str = "fuga_stack.bin", steps: int = 30) -> list[str]:

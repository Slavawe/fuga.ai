...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            "name": "fuga_query",
            "description": (
                "Query Fuga's trained knowledge base (VSA hyperdimensional memory over code in 5 languages, "
                "physics, Fuga's own source, and agent-injected facts). Use this for domain knowledge retrieval "
                "before writing code: e.g. how a known library/pattern works, physics/architecture background, "
                "or past decisions. Returns top matches with source path and snippet."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "question": {"type": "string", "description": "Question or keywords for the knowledge base"},
                    "top_k": {"type": "number", "description": "How many matches to return (default 8, max 25)"},
                },
                "required": ["question"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "fuga_learn",
            "description": (
                "Inject a new knowledge fact into Fuga's corpus (fuga_injections.jsonl). Fuga learns through "
                "these injections: the fact is folded into the unified training stack on the next train-stack run. "
                "Use when you discover a durable fact worth remembering (a decision, a gotcha, a domain insight) "
                "that should persist across sessions beyond necli memory."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "text": {"type": "string", "description": "The fact/knowledge to inject, self-contained text"},
                    "title": {"type": "string", "description": "Short name for the fact (optional)"},
                    "language": {"type": "string", "description": "Content language hint (default 'en')"},
                },
                "required": ["text"],
            },
        },

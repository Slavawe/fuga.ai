...
fn test_moe_routing_purity() {
    let tests = vec![
        ...
        ("impl MemoryStore for WaveCube { fn store(&mut self) {} }", "code"),
    ...
    for (query, expected) in &tests {
        ...
        let ok = got == *expected;
        if ok { correct += 1; }
        eprintln!("  [{}] '{:.50}' → {:?} (expected {:?})",
            if ok { "✓" } else { "✗" }, query, got, expected);
        assert_eq!(got, *expected,
            "routing mismatch for: {}", query);
    }
...
fn test_moe_routing_no_overlap_code_vs_dialogue() {
    ...
    for q in &code_queries {
        assert_eq!(MoEStore::domain_for(q), "code", "code query misrouted: {}", q);
    }
    for q in &dialogue_queries {
        assert_eq!(MoEStore::domain_for(q), "dialogue", "dialogue query misrouted: {}", q);
    }
}

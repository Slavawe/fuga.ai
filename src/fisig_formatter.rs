...
pub fn format_answer<const N: usize, const S: usize>(
    ai: &mut FugaAI<N, S>,
    query: &str,
    tokens: &[TokenInfo],
) -> FisigAnswer {
...
fn extract_core_theorem(results: &[(String, f64, String)], query: &str) -> String {
    ...
    if let Some((_, _, best)) = results.first() {
        for line in best.lines() {
            ...
            if tl.contains("equation")
                || tl.contains("formula")
                || tl.contains("principle")
                || tl.contains("law")
                || tl.contains("theorem")
                || tl.contains("delta rho_0")
                || tl.contains("ds^2")
                || tl.contains("partial")
                || tl.contains("nabla")
                || tl.contains("integral")
                || tl.contains("epsilon")
            {
    ...
    } else if q.contains("aether")
        || q.contains("ether")
        || q.contains("newton") && q.contains("gradient")
    {
        if theorem.is_empty() {
            theorem.push_str(
...
fn extract_derivation_steps(results: &[(String, f64, String)], query: &str) -> Vec<String> {
    ...
    if q.contains("mach") || q.contains("woodward") {
        steps.push(
            "Step 1: Start from Einstein field equations with Machian boundary conditions"
                .to_string(),
        );
        ...
        steps.push(
            "Step 6: Contract to Ricci: R_{ij} = R^k_{ikj}. Contract again: R = g^{ij}R_{ij}"
                .to_string(),
        );
        steps.push(
            "Step 7: Einstein tensor G_{μν} = R_{μν} − ½Rg_{μν} is divergence-free".to_string(),
        );
    } else if q.contains("qft")
        || q.contains("energy condition")
        || q.contains("anec")
        || q.contains("quantum inequality")
    {
        ...
        steps.push(
            "Step 2: Compute renormalized expectation ⟨T_{μν}⟩ via point-splitting".to_string(),
        );
        ...
        steps.push(
            "Step 6: For Alcubierre τ₀ ≈ R/γv ⇒ constraint ρRτ₀ ≤ 1 in Planck units".to_string(),
        );
    } else if q.contains("z3") || q.contains("formal verification") || q.contains("smt") {
        steps.push(
            "Step 1: Encode problem in SMT-LIB 2: declare-fun + assert + check-sat".to_string(),
        );
        steps.push(
            "Step 2: Z3 applies DPLL(T): propositional SAT modulo theory solvers".to_string(),
        );
        steps.push(
            "Step 3: Nelson-Oppen theory combination for uninterpreted functions + arithmetic"
                .to_string(),
        );
        ...
        steps.push(
            "Step 6: If UNSAT → reject parameter vector at compile time with diagnostic"
                .to_string(),
        );
        ...
        for line in best.lines() {
            ...
            if tl.contains("equation")
                || tl.contains("derive")
                || tl.contains("therefore")
                || tl.contains("thus")
                || tl.contains("from")
                || tl.contains("substituting")
                || tl.contains("integrating")
                || tl.contains("differentiating")
                || tl.contains("expanding")
                || tl.contains("solving")
            {
...
fn extract_limits(results: &[(String, f64, String)], query: &str) -> Vec<String> {
    ...
    if q.contains("mach") || q.contains("woodward") {
        ...
        limits.push(
            "Exotic matter constraint: ANEC must be satisfied for all null geodesics".to_string(),
        );
        ...
        limits
            .push("v > c: requires negative energy density ρ < −c²/R² in Planck units".to_string());
        ...
        limits.push(
            "Energy dominance: Tμν violates weak, strong, and null energy conditions".to_string(),
        );
    } else if q.contains("aether") || q.contains("ether") || q.contains("newton") {
        ...
        limits.push(
            "M → M_max: gradient exceeds critical; aether cavitation (black hole)".to_string(),
        );
        ...
        limits.push(
            "Gödel Incompleteness: ZFC cannot prove its own consistency (2nd theorem)".to_string(),
        );
        ...
        limits.push(
            "Large cardinal axioms: required for certain category-theoretic constructions"
                .to_string(),
        );
        ...
        limits.push(
            "Topology change: forbidden if metric is globally hyperbolic (Geroch theorem)"
                .to_string(),
        );
        limits.push("Singularity: R_μνρσ R^μνρσ → ∞ at curvature singularity".to_string());
        limits.push(
            "ADM energy: positive if dominant energy condition holds (Witten proof)".to_string(),
        );
    } else if q.contains("qft")
        || q.contains("quantum field")
        || q.contains("energy condition")
    ...
    {
        limits.push(
            "WEC: T_μν u^μ u^ν ≥ 0 — violated by Casimir vacuum (ρ < 0 between plates)".to_string(),
        );
        limits.push("NEC: T_μν k^μ k^ν ≥ 0 — violated by any warp drive geometry".to_string());
        limits.push(
            "SEC: (T_μν − ½Tg_μν) u^μ u^ν ≥ 0 — violation required for inflation".to_string(),
        );
        limits.push("DEC: T_μν u^μ is causal — violated by phantom energy (w < −1)".to_string());
        limits.push(
            "Ford-Roman bound: negative energy pulses limited in magnitude × duration".to_string(),
        );
        ...
        limits.push(
            "DPLL(T) performance: exponential in worst case; heuristic-dependent".to_string(),
        );
        limits.push("Array theory: extensionality makes decision problem harder".to_string());
        limits.push(
            "Bitvector: decidable but exponential in width; use incremental solving".to_string(),
        );
        limits.push("Alive2: only verifies LLVM IR, not source-level Rust semantics".to_string());
        limits.push(
            "Crucible: sound for a subset of Rust; unsafe code requires manual annotations"
                .to_string(),
        );
    }
    ...
    if let Some((_, _, best)) = results.first() {
        for line in best.lines() {
            let tl = line.to_lowercase();
            if tl.contains("bound")
                || tl.contains("constraint")
                || tl.contains("limit")
                || tl.contains("singularity")
                || tl.contains("cannot")
...
pub fn render_fisig_answer(answer: &FisigAnswer) -> String {
    ...
    let labels: Vec<String> = match answer.context_tag.as_str() {
        ...
        .map(String::from)
        .collect(),
        "warp" => vec!["R [m]", "v [m/s]", "σ [m⁻¹]", "ρ_min [kg/m³]", "x_s [m]"]
            .into_iter()
            .map(String::from)
            .collect(),
        "aether" => vec!["M [kg]", "r [m]", "ρ₀ [kg/m³]", "G [m³/kg·s²]", "c [m/s]"]
            .into_iter()
            .map(String::from)
            .collect(),
        "tesla" => vec!["f [Hz]", "V [V]", "L [m]", "ε₀ [F/m]"]
            .into_iter()
            .map(String::from)
            .collect(),
        "zfc" => vec![
        ...
        .map(String::from)
        .collect(),
        "geometry" => vec!["dim", "n_riemann", "n_ricci", "R [scalar curvature]", "Λ"]
            .into_iter()
            .map(String::from)
            .collect(),
        "qft" => vec!["WEC", "NEC", "SEC", "DEC", "ANEC_sat", "τ₀ [s]"]
            .into_iter()
            .map(String::from)
            .collect(),
        "z3" => vec!["sat?", "verifiable?", "n_vars", "timeout [s]"]
            .into_iter()
            .map(String::from)
            .collect(),
        _ => (0..answer.system_vector.len())
            .map(|i| format!("p{}", i))
            .collect(),
    };

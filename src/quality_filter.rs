...
use crate::multi::{LanguageId, MultiChaosLayer, MultiSemanticLayer, MultiSyntaxLayer};
use std::path::Path;
...
impl CodeQualityFilter {
    ...
    fn analyze_rust(&mut self, source: &str, path: &str) -> Result<QualityScore, String> {
        let engine = self
            .fuga_engine
            .get_or_insert_with(|| FugaEngine::new(self.dim));
        let result = engine
            .analyze(source)
        ...
        let bug_detected = result
            .fuga_result
            .as_ref()
            .map(|r| r.bug_detected)
            .unwrap_or(false);
        let coherence = result.layer_results.semantic.coherence;

        let weight = QualityScore::calculate_weight(
            safety,
            coherence,
            violations,
            attacks,
    ...
    fn analyze_multi(
        ...
        let raw_weight = QualityScore::calculate_weight(
            safety,
            coherence,
            violations,
            attacks,
    ...
    pub fn scan_directory(
        ...
        dir: &str,
        recursive: bool,
    ) -> Result<Vec<(String, QualityScore)>, String> {
        use walkdir::WalkDir;

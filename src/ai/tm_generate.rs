...
/// Learn a byte-transition on a KAN operator (KAN-lite, the nonlinear
/// replacement for the linear `W` on the SAME local window→next latent
/// prediction). `window_bytes` → `next_byte`: x = encoder(structure_sdr(window)),
/// target = encoder(byte_basis(next)); KAN's Widrow-Hoff spline update fits
/// the pair. Meant to be called in the corpus training loop exactly like
/// `TemporalMemory::learn_bytes`, but writing into `kan` instead of the linear W.
...
/// KAN-lite byte decoder: same 256-byte cosine ranking as the linear path, but
/// the transition operator is the spline-based `KanTransition` instead of the
/// linear `W`. `pred = kan.apply(encoder(structure_sdr(window)))`. This isolates
/// the roadmap hypothesis: a nonlinear low-parameter operator can split the
/// interleaved attractors (e→r vs structural) that the linear W cannot
/// (proven on synthetic data in kan.rs).
pub fn tm_generate_kan(
    ...
    while out.len() < steps && guard < steps * 2 {
        guard += 1;
        let window_sdrs: Vec<SdrVector> = window.iter().map(|&b| crate::ai::sdr::byte_basis(b)).collect();
        let x = encoder.encode(&crate::ai::sdr::structure_sdr_from_sdrs(&window_sdrs));

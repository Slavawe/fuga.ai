...
pub fn tm_generate_kan(
    ...
    while out.len() < steps && guard < steps * 2 {
        ...
        out.push(byte);
        window.push(byte);
        // Окно держится РОВНО window_size байт: без trim фолд растёт с каждым
        // шагом и уходит из обученного распределения (обучение data[i-ctxw..=i]).
        let wcap = window_size.max(1);
        if window.len() > wcap {
            window.drain(..window.len() - wcap);
        }
    }
    out
}
...
pub fn tm_generate_cosine_gate_v2(
    ...
    while out.len() < max_bytes && guard < max_bytes * 2 {
        ...
        // Хвост истории в ПРЯМОМ порядке (старые→новые): windows(2)/(3) ниже
        // ищут вхождения биграммы (t0,b) / триграммы (t1,t0,b) как есть.
        let recent: Vec<u8> = {
            let start = state.len().saturating_sub(24);
            state[start..].to_vec()
        };
...
pub fn tm_generate_hybrid(
    ...
    while out.len() < steps && guard < steps * 2 {
        ...
        out.push(byte);
        window.push(byte);
        // Окно держится РОВНО window_size байт (см. tm_generate_kan): без trim
        // вход гибрида растёт и расходится с окном обучения.
        let wcap = window_size.max(1);
        if window.len() > wcap {
            window.drain(..window.len() - wcap);
        }
...
while out.len() < max_bytes && guard < max_bytes * 4 {
        ...
        let patch_window: Vec<&[u8]> = patches_v
            .iter()
            .rev()
            .take(8) // v8.1 ГОРИЗОНТ: 8 патчей = 16 байт (как обучение)
            .rev()
            .map(|p| p.as_slice())
            .collect();
        let mut pp = crate::ai::latent_jepa::LatentVector::zero();
...
pub fn tm_generate_megabyte_v3(
    tm: &TemporalMemory,
    seed_bytes: &[u8],
    max_bytes: usize,
    window_bytes: usize,
...
while out.len() < max_bytes && guard < max_bytes * 4 {
        ...
        // JEPA MACRO: z_macro = W_macro·x_ctx — семантическое намерение.
        // x_ctx = латент БАЙТОВОГО окна (как обучение W_macro: encode(окно)).
        // Обрезаем до window_bytes: рост окна рассинхронизирует базис с тренингом
        // (аудит 22.08: z_macro на всей истории).
        let mut z_macro: Option<crate::ai::latent_jepa::LatentVector> = None;
        if !macro_w.is_empty() && beta_macro > 0.0 {
            let macro_win_bytes = window_bytes.max(1);
            let macro_lo = window.len().saturating_sub(macro_win_bytes);
            let win_sdrs: Vec<crate::ai::sdr::SdrVector> = window[macro_lo..]
                .iter()
                .map(|&b| crate::ai::sdr::byte_basis(b))
                .collect();
            let x_ctx = encoder.encode(&crate::ai::sdr::structure_sdr_from_sdrs(&win_sdrs));
            let mut zm = crate::ai::latent_jepa::LatentVector::zero();
            for o in 0..crate::ai::latent_jepa::LATENT_DIM {
                let row = o * crate::ai::latent_jepa::LATENT_DIM;
                let mut acc = 0.0f32;
                for i in 0..crate::ai::latent_jepa::LATENT_DIM {
                    acc += macro_w[row + i] * x_ctx.values[i];
                }
                zm.values[o] = acc;
            }
            let zn = zm.values.iter().map(|v| v * v).sum::<f32>().sqrt().max(1e-8);
            for v in &mut zm.values {
                *v /= zn;
            }
            z_macro = Some(zm);
        }

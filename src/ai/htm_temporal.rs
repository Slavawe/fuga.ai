...
impl TemporalMemory {
    ...
    pub fn load(path: &str) -> Option<Self> {
        ...
        // Guard-ридеры (аудит 22.08): усечённый/малформированный файл обязан
        // давать None, а не панику slice out of range; срез — ТОЛЬКО после
        // проверки границы. Счётчики из файла ограничивают with_capacity
        // физически возможным числом записей (иначе 4 байта 0xFF заказывают
        // сотни ГБ и убивают процесс аллокатором).
        ...
        let rd_f64 = |data: &[u8], pos: &mut usize| -> Option<f64> {
            if *pos + 8 > data.len() {
                return None;
            }
        ...
        // Каждая клетка в файле ≥ 1032 байта (id + 1024 bits + seg_n).
        let mut cells = Vec::with_capacity(n.min(data.len() / 1032 + 1));
        for _ in 0..n {
            let id = rd_u32(&data, &mut pos)? as usize;
            let mut bits = [0u64; 128];
            for w in bits.iter_mut() {
                *w = rd_u64(&data, &mut pos)?;
            }
            let pattern = crate::ai::sdr::SdrVector { bits };
            let seg_n = rd_u32(&data, &mut pos)? as usize;
            let mut segments = Vec::with_capacity(seg_n.min(data.len() / 4 + 1));
            for _ in 0..seg_n {
                let syn_n = rd_u32(&data, &mut pos)? as usize;
                let mut synapses = Vec::with_capacity(syn_n.min(data.len() / 12 + 1));
                for _ in 0..syn_n {
                    let bi = rd_u32(&data, &mut pos)? as usize;
                    let perm = rd_f64(&data, &mut pos)?;
                    // bit_index за пределами SDR_DIM даёт отложенный OOB в
                    // overlap/reinforce — невалидный синапс пропускаем.
                    if bi < crate::ai::sdr::SDR_DIM {
        ...
        let wl = rd_u32(&data, &mut pos)? as usize;
        // Каждая запись окна в файле = 1024 байта.
        let mut window = Vec::with_capacity(wl.min(data.len() / 1024 + 1));
        for _ in 0..wl {
            let mut bits = [0u64; 128];
            for w in bits.iter_mut() {
                *w = rd_u64(&data, &mut pos)?;

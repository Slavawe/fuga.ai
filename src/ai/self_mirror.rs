...
impl SelfMirror {
        ...
        if let Ok(data) = std::fs::read(node_path) {
            ...
            if pos + 4 <= data.len() {
                ...
                for _ in 0..n {
                    ...
                    let path =
                        String::from_utf8(data[pos..pos + plen].to_vec()).unwrap_or_default();
                    pos += plen;
                    if pos + 4 > data.len() {
                        break;
                    }
                    ...
                    let kind =
                        String::from_utf8(data[pos..pos + klen].to_vec()).unwrap_or_default();
                    pos += klen;
                    if pos + 4 > data.len() {
                        break;
                    }
                    ...
                    let name =
                        String::from_utf8(data[pos..pos + nlen].to_vec()).unwrap_or_default();
                    pos += nlen;
                    if pos + 24 > data.len() {
                        break;
                    }
                    ...
                    let language = if pos + 4 <= data.len() {
                        ...
                        if pos + llen <= data.len() {
                            let lang = String::from_utf8(data[pos..pos + llen].to_vec())
                                .unwrap_or_default();
                            pos += llen;
                            lang
                        } else {
        ...
        if let Ok(data) = std::fs::read("fuga_token_vocab.bin") {
            ...
            if pos + 4 <= data.len() {
                ...
                for _ in 0..n {
                    ...
                    let tlen = u32::from_le_bytes(data[pos..pos + 4].try_into().unwrap_or([0u8; 4]))
                        as usize;
                    pos += 4;
    ...
    pub fn evaluate(&mut self) -> String {
        ...
        for fp in &paths {
            ...
            for chunk in words.chunks(10) {
                ...
                let hv = crate::ai::temporal_predictor::sdr_to_hypervector(
                    &tm_pred,
                    self.predictor.hjepa.dim,
                );
                ...
                let ctx: Vec<&Hypervector> = self.predictor.buffer
                    [..self.predictor.buffer.len() - 1]
                    .iter()
                    .collect();
                ...
                if full_pred.len() > 2 {
                    ...
                    let l1_sim = self.predictor.hjepa.levels[1]
                        .similarity_to_expected(&ctx[l1_start..], actual);
                    total_l1 += 1.0 - l1_sim;
                    sim_l1 += l1_sim;
        ...
        println!(
            "  modes={:?} avg_sim_L0={:.4} avg_sim_L1={:.4}",
            dbg_modes,
            a(sim_l0, total_cnt),
            a(sim_l1, total_cnt)
        );
        format!(
            "L0={:.4} L1={:.4} L2={:.4} samples={}",
            a(total_l0, total_cnt),
            a(total_l1, total_cnt),
            a(total_l2, total_cnt),
            total_cnt
        )
    }
    ...
    pub fn generate_code_autoregressive(
        &mut self,
        seed: &str,
        max_steps: usize,
        _temperature: f64,
        ...
        for word in seed.split_whitespace() {
            ...
            let hv =
                crate::ai::temporal_predictor::sdr_to_hypervector(&sdr, self.predictor.hjepa.dim);
            self.predictor.buffer.push(hv);
            if self.predictor.buffer.len() > self.predictor.buf_capacity {
                self.predictor.buffer.remove(0);
            }
        ...
        let goal_hv =
            crate::ai::temporal_predictor::sdr_to_hypervector(&goal_sdr, self.predictor.hjepa.dim);
        ...
        let is_rust_seed = seed.contains("fn ")
            || seed.contains("impl ")
            || seed.contains("struct ")
            || seed.contains("async ");
        let node_sdr_cache: Vec<(String, SdrVector, String, usize, usize)> = self
            .nodes
            .iter()
            .map(|n| {
                (
        ...
        for _ in 0..max_steps {
            ...
            for (npath, nsdr, nlang, nforder, nline) in &node_sdr_cache {
                ...
                if recent_node_keys.iter().any(|k| *k == node_key) {
                    continue;
                }
            ...
            if best_overlap_raw < 10 {
                ...
                for (npath, nsdr, _nlang, _nforder, nline) in &node_sdr_cache {
                    ...
                    if o > retry_overlap {
                        retry_overlap = o;
                        retry_path = npath.clone();
                        retry_lineno = *nline;
                    }
                }
                if retry_overlap > best_overlap_raw {
                    best_overlap_raw = retry_overlap;
                    best_path = retry_path;
                    best_lineno = retry_lineno;
                }
            }
            if dominant_lang.is_none() {
                let candidate = self.nodes.iter().find(|n| n.path == best_path);
                if let Some(n) = candidate {
                    dominant_lang = Some(n.language.clone());
                }
            }
            ...
            let combined = if snippet.is_empty() {
                format!(
                    "// (source not available) {} overlap={} score={:.0}",
                    best_path, best_overlap_raw, best_score
                )
            } else {
                format!(
                    "// overlap={} score={:.0}\n{}",
            ...
            if last_nodes.iter().filter(|x| *x == &combined).count() >= 3 {
                break;
            }
            recent_node_keys.push(format!("{}:{}", best_path, best_lineno));
            if recent_node_keys.len() > 8 {
                recent_node_keys.remove(0);
            }
            let node_sdr = encode_text(&snippet);
            self.predictor.tm.feed(&node_sdr);
            let node_hv = crate::ai::temporal_predictor::sdr_to_hypervector(
                &node_sdr,
                self.predictor.hjepa.dim,
            );
    ...
    pub fn generate_tokens(&mut self, seed: &str, max_tokens: usize) -> Vec<String> {
        ...
        for _ in 0..max_tokens {
            ...
            for (tok, tsdr) in &self.token_vocab {
                if recent_set.iter().any(|t| t == tok) {
                    continue;
                }
    ...
    pub fn train_token_sequences(&mut self, max_sequences: usize, steps_per_seq: usize) -> usize {
        ...
        let rust_keywords: std::collections::HashSet<&str> = [
            "fn", "let", "mut", "pub", "struct", "impl", "use", "mod", "type", "enum", "match",
            "if", "else", "while", "loop", "for", "in", "return", "self", "Self", "as", "break",
            "continue", "const", "static", "ref", "where", "unsafe", "trait", "impl", "dyn",
            "move", "box", "await", "async",
        ]
        .into_iter()
        .collect();
        let mut paths: std::collections::BTreeSet<String> = self
            ...
            .iter()
            .map(|n| n.path.clone())
            .filter(|p| p.ends_with(".rs") && allowed_dirs.iter().any(|d| p.contains(d)))
            .collect();
        ...
        let syntax_patterns: Vec<Vec<&str>> = vec![
            ...
            vec![
                "match", "res", "{", "Ok", "(", "v", ")", "=>", "v", ",", "Err", "(", "e", ")",
                "=>", "e", "}",
            ],
            vec![
                "impl", "Foo", "{", "pub", "fn", "bar", "(", ")", "->", "i32", "{", "0", "}", "}",
            ],
            vec![
                "for", "item", "in", "list", ".", "iter", "(", ")", "{", "println", "!", "(", ")",
                ";", "}",
            ],
            vec![
            ...
            vec!["fn", "audit", "(", "&", "self", ")", "{", "}"],
            vec![
                "fn", "process", "(", "input", ":", "&", "str", ")", "->", "bool", "{", "true", "}",
            ],
    ...
    pub fn generate_code_beam(
        &mut self,
        text: &str,
        steps: usize,
        beam_width: usize,
        temperature: f64,
    ) -> Vec<Vec<(PhaseNode, u32)>> {
    ...
    pub fn reflect(&self) -> String {
        ...
        format!(
            "mirror: {} nodes ({}fn {}struct {}impl) avg_err={:.4} cells={}",
            self.nodes.len(),
            fns,
            structs,
            impls,
    ...
    fn extract_c_fn(line: &str) -> Option<String> {
        ...
        if name.chars().all(|c| c.is_uppercase() || c == '_') && name.chars().any(|c| c == '_') {
            return None;
        }
    ...
    pub fn index_file_c(&mut self, path: &str) -> Vec<PhaseNode> {
        ...
        while i < lines.len() {
            ...
            if let Some((name, body)) = Self::extract_macro(trimmed) {
                let (tm_match, errors) = self
                    .predictor
                    .feed_learn(&format!("#define {} {}", name, body));
                file_counter += 1;
            ...
            if let Some(tag) = Self::extract_ifdef(trimmed) {
                let (tm_match, errors) =
                    self.predictor.feed_learn(&format!("#preprocessor {}", tag));
                file_counter += 1;
                nodes.push(PhaseNode {
                    path: path.to_string(),
                    line: i + 1,
...
impl SelfMirror {
    ...
    fn train_predictor_inner(
        ...
        println!(
            "  Re-training HJEPA on {} files ({} epochs, chunk_size={}, ff={})...",
            count, epochs, cs, use_ff
        );
        ...
        println!(
            "  Estimated chunks per epoch: {} (total across {} epochs: {})",
            est_chunks,
            epochs,
            est_chunks * epochs
        );
        ...
        for epoch in 0..epochs {
            ...
            if skip_tm {
                ...
                println!(
                    "  Pre-train weights: w0={:.2} w1={:.2} w2={:.2}",
                    pre_w0, pre_w1, pre_w2
                );
                for (fi, fp) in files.iter().enumerate() {
                    ...
                    if (fi + 1) % 20 == 0 {
                        print!(
                            "\r  Fast: {}/{} files  chunks={}  loss={:.4}",
                            fi + 1,
                            200,
                            epoch_cnt,
                            epoch_loss / epoch_cnt.max(1) as f64
                        );
                        use std::io::{Write, stdout};
                        stdout().flush().ok();
                    }
                ...
                println!(
                    "\r  Trained: {} files, {} chunks  loss={:.4}  w=[{:.2} {:.2} {:.2}] → [{:.2} {:.2} {:.2}] Δ0={:.4}",
                    200,
                    epoch_cnt,
                    epoch_loss / epoch_cnt.max(1) as f64,
                    pre_w0,
            ...
            total_cnt += 1;
            print!(
                "\r  Epoch {:3}/{}  chunk_size={} avg_loss={:.4}",
                epoch + 1,
                epochs,
                cs,
                avg
            ...
            *order += 1;
            ...
            if (idx + 1) % 500 == 0 {
                ...
                let segs: usize = self
                    ...
                    .sum();
                println!(
                    "  trained {}/{} ({:.0}%)  cells={} segments={}  (checkpoint)",
                    idx + 1,
                    total,
                    pct,
                    cells,
    ...
    pub fn inspect_file(&self, path: &str) -> InspectReport {
        ...
        let anomaly_score =
            1.0 - (profile.token_entropy / 10.0).min(1.0) + (1.0 - sdr_density / 0.02).abs();
    ...
    pub fn inspect_dir(&self, dir: &str) -> Vec<InspectReport> {
        ...
        for entry in entries {
            ...
            if ext == "rs" || Self::is_c_cpp(ps) || Self::is_java(ps) {
                ...
                println!(
                    "  {}  entropy={:.2} struct_entropy={:.2} density={:.3} anomaly={:.3}  → {}",
                    p.display(),
                    report.entropy,
                    report.struct_entropy,
                    report.sdr_density,
    ...
    pub fn evaluate_debug(&mut self) -> String {
        ...
        format!(
            "DBG sim(pred,actual)={:.4} sim(baseline,actual)={:.4} n={}",
            sim_pred / cnt as f64,
            sim_baseline / cnt as f64,
            cnt
        )
    }
}
...
impl AutoCorrectEngine {
    ...
    pub fn apply_correction(&mut self, text: &str) -> (String, Vec<String>) {
        ...
        if let Some(sugg) = suggestion {
            ...
            self.correction_history.push((
                text.to_string(),
                sugg.node.name.clone(),
                sugg.resonance,
            ));
        }

...
impl JepaLevel {
    ...
    pub fn grouped_topk_mask(&self, bcount: usize, scores: &[f64]) -> Vec<bool> {
        ...
        if self.top_k == 0 || bcount == 0 {
            return vec![true; bcount];
        }
        let ng = if self.num_expert_group > 1
            && self.topk_group > 0
            && self.topk_group < self.num_expert_group
            && self.num_expert_group <= bcount
        {
        ...
        if ng == 1 {
            ...
            for &i in idx.iter().take(self.top_k.min(bcount)) {
                keep[i] = true;
            }
            return keep;
        }
        ...
        for &g in gidx.iter().take(self.topk_group.min(ng)) {
            gkeep[g] = true;
        }
        ...
        for &i in cand.iter().take(self.top_k.min(cand.len())) {
            keep[i] = true;
        }
        keep
    }
    ...
    pub fn train_step_ff(
        &mut self,
        context: &[&Hypervector],
        actual: &Hypervector,
        negative: &Hypervector,
        _margin: f64,
    ) -> f64 {
        let dim = self.dim;
        let n_ctx = context.len().min(self.context_len);
        ...
        for (i, hv) in context.iter().take(n_ctx).enumerate() {
            for k in 0..PERM_EXPANSION {
                ...
                for bit in active_bits(&permuted.words, dim) {
                    ...
                    self.weights[w_idx] =
                        self.weights[w_idx] * (1.0 - decay) + lr * (t_pos - t_neg);
                }
            }
        }
    ...
    fn load(data: &[u8], offset: &mut usize) -> Result<Self, String> {
        ...
        *offset += 4;
        if *offset + nl > data.len() {
            return Err("short name".into());
        }
        let name = String::from_utf8(data[*offset..*offset + nl].to_vec())
            .map_err(|e| format!("utf8: {}", e))?;
        *offset += nl;
        if *offset + 8 > data.len() {
            return Err("short after name".into());
        }
        ...
        *offset += 4;
        ...
        let delta_ones = if has_delta_ones {
            if *offset + 4 > data.len() {
                return Err("short delta_ones".into());
            }
        ...
        let (num_expert_group, topk_group) = if has_grouped_topk {
            if *offset + 8 > data.len() {
                return Err("short grouped_topk".into());
            }
...
impl HierarchicalJEPA {
    ...
    pub fn predict_sequence_beam(
        ...
        let mut beams: Vec<(Vec<Hypervector>, f64)> =
            vec![(context.iter().map(|h| (*h).clone()).collect(), 0.0)];
        for _ in 0..steps {
            ...
            for (seq, score) in &beams {
                for _ in 0..beam_width {
                    ...
                    let l1_win: Vec<&Hypervector> = seq
                        [seq.len().saturating_sub(self.levels[1].context_len)..]
                        .iter()
                        .collect();
                    ...
                    let novelty = if entropy > 0.0 && entropy < 1.0 {
                        -(entropy * (entropy + 0.01).ln()
                            + (1.0 - entropy) * (1.0 - entropy + 0.01).ln())
                    } else {
                        0.0
                    };
        ...
        beams
            .into_iter()
            .map(|(seq, _)| seq[seq.len() - steps..].to_vec())
            .collect()
    }
    ...
    pub fn predict_refined(
        ...
        let chosen = if converge > 0.7 {
            let sdr_refs: Vec<_> = corrected_list
                .iter()
                .map(|h| crate::ai::sdr::sparsify(h))
                .collect();
    ...
    pub fn train_cross_domain(&mut self, pairs_path: &str, epochs: usize) -> f64 {
        ...
        let pairs: Vec<serde_json::Value> = data
            .lines()
            .filter_map(|l| serde_json::from_str(l).ok())
            .collect();
        println!(
            "  Loaded {} doc→code pairs from {}\n",
            pairs.len(),
            pairs_path
        );
        if pairs.is_empty() {
        ...
        let avg_sim = if final_cnt > 0 {
            final_sim / final_cnt as f64
        } else {
            0.0
        };
        println!(
            "  Cross-domain L1 similarity: {:.4} (1.0 = perfect alignment)\n",
            avg_sim
        );
        avg_sim
    }
    ...
    pub fn train_on_directory(&mut self, dir: &str, epochs: usize) -> f64 {
        let exts = &[
            ".rs", ".py", ".js", ".ts", ".c", ".cpp", ".h", ".go", ".java", ".toml", ".json",
            ".yaml", ".txt", ".md",
        ];
        ...
        for (fi, fp) in files.iter().enumerate() {
            ...
            let hv: Vec<Hypervector> = words
                .chunks(10)
                .map(|c| {
                    let token_hvs: Vec<Hypervector> = c
                        .iter()
                        .map(|w| {
                            let id = token_id(w);
        ...
        println!(
            "\n    Generated {} L1 sequences ({} predictions)",
            l1_seqs.len(),
            pred_count
        );
        ...
        println!(
            "\n    Generated {} L2 sequences ({} error deltas)",
            l2_seqs.len(),
            err_count
        );
        ...
        for epoch in 0..epochs {
            ...
            match state {
                ...
                SystemState::CriticalResetRequired => {
                    println!(
                        "\n    🔴 CB critical at epoch {} (loss={:.4}) — resetting L2 weights",
                        epoch + 1,
                        avg_loss
                    );
                    let mut rng = rand::thread_rng();
        ...
        let avg: Vec<f64> = (0..3)
            .map(|li| {
                if final_cnt[li] > 0 {
                    final_loss[li] / final_cnt[li] as f64
                } else {
                    0.5
                }
            })
        ...
        println!(
            "  Final eval: L0={:.4} L1={:.4} L2={:.4} avg={:.4}",
            avg[0], avg[1], avg[2], overall
        );
        overall
    }
    ...
    pub fn load(path: &str) -> Result<Self, String> {
        ...
        let ver = data[5];
        if ver != 1 {
            return Err(format!("unknown version {}", ver));
        }
        ...
        for _ in 0..n {
            levels.push(JepaLevel::load(&data, &mut off)?);
        }
...
fn str_chunk_hv(text: &str, dim: usize) -> Hypervector {
    ...
    let token_hvs: Vec<Hypervector> = words
        .iter()
        .map(|w| {
            let id = token_id(w);

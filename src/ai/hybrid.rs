...
impl HybridTransition {
    ...
    pub fn learn_pair(
        ...
        // 2) KAN-остаток: KAN учит НЕвыразимое линейно. Пересчитываем
        //    предсказание ПОСЛЕ W-дельты (как эталон vsa_jepa_kan.rs):
        //    иначе KAN учит то, что W только что выучил сам.
        let mut pred_w2 = LatentVector::zero();
        for o in 0..LATENT_DIM {
            let row = o * LATENT_DIM;
            let mut acc = 0.0f32;
            for i in 0..LATENT_DIM {
            ...
            pred_w2.values[o] = acc;
        }
        let mut residual = LatentVector::zero();
        let mut rn = 0.0f32;
        for o in 0..LATENT_DIM {
            residual.values[o] = t.values[o] - pred_w2.values[o];
            rn += residual.values[o] * residual.values[o];
        }
        let rn = rn.sqrt();
        // Гаснем на выученных переходах: не впрыскиваем единичный вектор
        // в шумовом направлении, когда остаток уже ≈ 0.
        if rn > 1e-3 {

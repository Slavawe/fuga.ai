...
impl KanTransition {
    ...
    /// декода за 431s. Формула: scale = min(1, sqrt(CAP/sq)) —
    /// при sq ≤ CAP строку НЕ трогаем (плавный sqrt(CAP/(CAP+sq)) резал
    /// на каждом капе даже малые нормы → нетренируемые узлы экспоненциально
    /// распадались, аудит 22.08); при sq > CAP — пропорциональный скейл
    /// к порогу, направление строки сохраняется (как cap_w в latent_jepa).
    /// WGSL KAN_CAP_SHADER в gpu_ops — СИНХРОННО с этой формулой.
    pub fn cap_outputs(&mut self) {
        ...
        for o in 0..LATENT_DIM {
            ...
            // Пороговый renormalizer: ниже CAP — нейтрально (scale=1),
            // выше — плавно тянет к CAP, не зануляя и не искажая направление.
            let scale = (NORM_CAP / sq.max(1e-8)).sqrt().min(1.0);

...
const STOP_WORDS: &[&str] = &[
    "the", "and", "for", "that", "this", "with", "from", "was", "are", "you",
    "not", "have", "has", "its", "but", "than", "them", "then", "they", "were",
    "will", "into", "more", "most", "some", "would", "their", "there", "which",
    "been", "what", "when", "where", "your", "is", "of", "to", "in", "if", "a",
    "or", "as", "on", "at", "it", "we", "he", "she", "his", "her", "be",
    "def", "return", "return", "use", "using", "self", "std", "const", "var",
    "new", "let", "for", "while", "this", "true", "false", "null", "none",
];
...
pub fn encode_text(text: &str) -> SdrVector {
    ...
    let tid_sdr: Vec<(u32, SdrVector)> = words.iter().map(|w| {
    ...
    let mut scored: Vec<(usize, u64)> = (0..SDR_DIM)
        ...
        .collect();
    scored.sort_by(|a, b| {
        freq[a.0].cmp(&freq[b.0]).then_with(|| a.1.cmp(&b.1))
    });

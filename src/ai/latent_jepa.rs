...
/// Cache of `SdrEncoder::encode` results keyed by (seed, input SDR). The
/// encoder projection is O(active_bits × LATENT_DIM) with a hash per cell, and
/// the structural training path re-encodes heavily overlapping windows (8 of 9
...
///
/// ВАЖНО: seed в ключе обязателен — encode зависит от seed энкодера, и кеш без
/// него тихо подсовывает латент ЧУЖОГО базиса при двух энкодерах в одном
/// процессе (cargo test гоняет seed 0xF03D_C0DE/0x9E37…/тестовые вместе).
static LATENT_ENC_CACHE: std::sync::LazyLock<std::sync::Mutex<std::collections::HashMap<(u64, SdrVector), LatentVector>>> =
...
impl SdrEncoder {
    ...
    pub fn encode(&self, sdr: &SdrVector) -> LatentVector {
        if let Ok(cache) = LATENT_ENC_CACHE.lock() {
            if let Some(cached) = cache.get(&(self.seed, sdr.clone())) {
                return cached.clone();
            }
        }
        ...
        if let Ok(mut cache) = LATENT_ENC_CACHE.lock() {
            ...
            cache.insert((self.seed, sdr.clone()), out.clone());
        }
        out
    }

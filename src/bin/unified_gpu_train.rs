...
fn main() {
    ...
    // CPU/GPU конвейер: --threads N > 1 параллелит кодирование пар (SDR →
    // латенты) на N ядер — узкое место конвейера был один поток-подготовщик,
    // GPU простаивал (шаг 5 AGENTS). Порядок пар сохраняется → бит-в-бит
    // тот же результат, что при --threads 1.
    ...
    // Валидационный сплит (план 22.08, этап 1.4): --val <val.bin> от data_prep.
    // На каждом чекпоинте — метрики БЕЗ обновлений: mean ||t − W·x|| и
    // mean cos(t, W·x) — контроль оверфитта (v6/v7-финалы переобучались,
    // решение о ранней остановке принималось вручную по декоду).
    let val_path: String = arg(&args, "--val", "".into());
    let val_data: Option<Vec<u8>> = if val_path.is_empty() {
        None
    } else {
        match std::fs::read(&val_path) {
            Ok(v) => {
                eprintln!(
                    "[val] {} — {:.1} MB (контроль оверфитта на чекпоинтах)",
                    val_path,
                    v.len() as f64 / 1e6
                );
                Some(v)
            ...
            Err(e) => {
                eprintln!("[val] {} не читается: {} — валидация выключена", val_path, e);
                None
            }
        }
    };
    ...
    let cpu_handle = {
        ...
        std::thread::spawn(move || {
            ...
            // Чистая функция пары (та же логика, что была инлайн): поток
            ...
            let make_pair = |data: &[u8], i: usize| -> Pair5 {
                ...
                // Byte-H-JEPA МАКРО: z_tgt = латент СЛЕДУЮЩЕГО окна.
                ...
                // v6: ПАТЧЕВОЕ ОКНО 1:1 С V2-ДЕКОДЕРОМ, 100% градиентный поток.
                let mut x2 = Vec::new();
                let mut t2 = Vec::new();
            ...
            'outer: for corpus_file in &corpora {
                ...
                // BIN-режим (data_prep, план 22.08): файл .bin — сырой байтовый
                // поток без JSON. Читаем чанками по 60KB (под лимит монстров
                // ниже) с перекрытием 2*ctxw+2, чтобы окна и макро-цели не
                // рвались на границах чанков.
                let is_bin = path
                    .extension()
                    .and_then(|e| e.to_str())
                    .map_or(false, |e| e.eq_ignore_ascii_case("bin"));
                // Итератор документов: bin → чанки; иначе строки JSONL.
                enum Doc {
                    Bin(Vec<u8>),
                    Line(String),
                }
                let docs: Box<dyn Iterator<Item = Doc>> = if is_bin {
                    let all = std::fs::read(path).unwrap_or_default();
                    let overlap = 2 * ctxw + 2;
                    let mut off = 0usize;
                    eprintln!(
                        "[bin] {} — {:.1} MB, чанки 60KB (overlap {}B)",
                        corpus_file,
                        all.len() as f64 / 1e6,
                        overlap
                    );
                    Box::new(std::iter::from_fn(move || {
                        ...
                        let end = (off + 60_000).min(all.len());
                        let chunk = all[off..end].to_vec();
                        off = if end < all.len() { end - overlap } else { end };
                        Some(Doc::Bin(chunk))
                    }))
                } else {
                    let f = std::fs::File::open(path).unwrap();
                    let reader = std::io::BufReader::new(f);
                    Box::new(reader.lines().map(|l| Doc::Line(l.unwrap_or_default())))
                };
                ...
                for doc in docs {
                    ...
                    // статистики не прибавляется, только время.
                    if data.len() > 65_536 {
                        continue;
                    }
                    // AST-маска ВЫКЛЮЧЕНА (bypass) ещё с fisig-инцидента:
                    ...
                    if threads > 1 {
                        ...
                        // диапазону) → поток обучения бит-в-бит идентичен
                        // --threads 1. Память блока ≈ 2048 × 7 латентов × 2KB
                        // ≈ 28MB — bounded.
    ...
    match &gpu {
        Some(g) => {
            ...
            // Перекрывающиеся окна хранятся ВМЕСТЕ со своей целью t2 —
            // окно сдвига −1 предсказывает тот же next_patch своей пары.
            let mut xs2b: Vec<(Vec<f32>, Vec<f32>)> = Vec::with_capacity(batch);
                ...
                match recv.recv() {
                    Ok((x, t, x2, t2, x2b, xm, tm)) => {
                        ...
                        if xs.len() >= batch {
                            ...
                            // Каждое окно учится на цели СВОЕЙ пары (как CPU-fallback).
                            let lam2 = lr_patch * lambda_now(applied) * 0.5;
                            for (xi, ti) in xs2b.iter() {
                                g.hybrid_step2(xi, ti, lam2);
                            }
                            applied += xs.len();
                            ...
                            if ckpt_every > 0 && applied >= next_ckpt {
                                ...
                                // Val-сплит: честный контроль оверфитта —
                                // предсказания БЕЗ обновлений весов.
                                if let Some(vd) = &val_data {
                                    if vd.len() > ctxw + 2 {
                                        let n_max = 4096usize;
                                        let span = vd.len() - ctxw - 1;
                                        let stride = (span / n_max).max(1);
                                        let (mut se, mut st, mut sc, mut cnt) =
                                            (0.0f64, 0.0f64, 0.0f64, 0usize);
                                        for k in (0..span).step_by(stride).take(n_max) {
                                            let win = &vd[k.saturating_sub(ctxw)..=k];
                                            let wsdrs: Vec<fuga::SdrVector> = win
                                                .iter()
                                                .map(|&c| byte_cache[c as usize].clone())
                                                .collect();
                                            let x = enc.encode(&structure_sdr_from_sdrs(&wsdrs));
                                            let t = enc.encode(&byte_cache[vd[k + 1] as usize]);
                                            let mut pred = [0.0f32; DIM];
                                            for o in 0..DIM {
                                                let row = o * DIM;
                                                let mut acc = 0.0f32;
                                                for i in 0..DIM {
                                                    acc += cw[row + i] * x.values[i];
                                                }
                                                pred[o] = acc;
                                            }
                                            let (mut e2, mut t2, mut dot, mut pn) =
                                                (0.0f32, 0.0f32, 0.0f32, 0.0f32);
                                            for o in 0..DIM {
                                                let d = t.values[o] - pred[o];
                                                e2 += d * d;
                                                t2 += t.values[o] * t.values[o];
                                                dot += t.values[o] * pred[o];
                                                pn += pred[o] * pred[o];
                                            }
                                            se += e2.sqrt() as f64;
                                            st += t2.sqrt() as f64;
                                            sc += (dot / (t2.sqrt() * pn.sqrt() + 1e-8)) as f64;
                                            cnt += 1;
                                        }
                                        if cnt > 0 {
                                            println!(
                                                "[val] {} пар (val): ||t−Wx||={:.4} (||t||={:.4}) cos(t,Wx)={:.4}",
                                                cnt,
                                                se / cnt as f64,
                                                st / cnt as f64,
                                                sc / cnt as f64
                                ...
                                let dec_t = fuga::tm_generate_latent_bytes(
                                    ...
                                    40,
                                    ctxw + 1, // окно декода == окно обучения data[i-ctxw..=i]
                                    None,
                                );
    ...
    for (label, seed) in [("TEXT", &seed_text), ("CODE", &seed_code)] {
        ...
        let o1 = fuga::tm_generate_latent_bytes(&tm_d, seed, 120, ctxw + 1, None);
        println!(
            "  naive       ({} B): {:?}",
            o1.len(),
        ...
        let o2a =
            fuga::tm_generate_two_speed_entropy(&tm_d, seed, 200, ctxw + 1, 0.60, &patch_vocab);
        println!(
            "  entropy-mem   ({} B): {:?}",
            o2a.len(),
            String::from_utf8_lossy(&o2a).chars().take(60).collect::<String>()
        );
        let o2b = fuga::tm_generate_two_speed_entropy(&tm_reload, seed, 200, ctxw + 1, 0.60, &patch_vocab);
        println!(
            "  entropy-file  ({} B): {:?}",
            o2b.len(),
            String::from_utf8_lossy(&o2b).chars().take(60).collect::<String>()
        );
        let o3 = fuga::ai::tm_generate::tm_generate_hybrid(&tm_d, &kan_d, seed, 120, ctxw + 1, 1.0);
        println!(
            "  hybrid W+K  ({} B): {:?}",
            o3.len(),

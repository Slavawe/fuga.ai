...
fn is_greeting(q: &str) -> bool {
    ...
    g == "привет" || g == "hello" || g == "hi" || g == "здравствуй"
        || g == "здравствуйте" || g == "даров" || g == "ку"
        || g.starts_with("привет ") || g.starts_with("hello ") || g.starts_with("hi ")
}

fn looks_like_code_request(text: &str) -> bool {
    let t = text.to_lowercase();
    t.len() >= 5
        && (t.contains("python") || t.contains("rust") || t.contains("javascript")
            || t.contains("js") || t.contains("go ") || t.contains("golang")
            || t.contains("c++") || t.contains("cpp") || t.contains("java")
            || t.contains("typescript") || t.contains("ts ")
            || t.contains("функция") || t.contains("класс") || t.contains("метод")
            || t.contains("code") || t.contains("program") || t.contains("script")
            || t.contains("напиши") || t.contains("создай") || t.contains("написать")
            || t.contains("файл") || t.contains("алгоритм") || t.contains("сортировк")
            || t.contains("server") || t.contains("api") || t.contains("endpoint")
            || t.contains("bot") || t.contains("функц") || t.contains("парс")
            || t.contains("regex") || t.contains("sql") || t.contains("запрос")
            || t.contains("db") || t.contains("баз") || t.contains("html")
            || t.contains("css") || t.contains("стил") || t.contains("апи")
            || t.contains("код"))
}

fn is_binary_question(text: &str) -> bool {
    let t = text.to_lowercase();
    t.contains("да или нет") || t.contains("yes or no") || t.contains("скажи да")
        || t.contains("ответь прямо") || t.contains("просто скажи")
        || t.contains("только да") || t.contains("только нет")
        || t == "да" || t == "нет" || t == "yes" || t == "no"
}
...
fn guess_language(text: &str) -> &'static str {
    let t = text.to_lowercase();
    if t.contains("fn ") || t.contains("impl ") || t.contains("let mut ")
        || t.contains(".rs") || t.contains("pub ") || t.contains("-> ")
    { "rust" }
    else if t.contains("def ") || t.contains("import ") || t.contains("print(")
        || t.contains("class ") || t.contains("self") || t.contains(".py")
    { "python" }
    else if t.contains("function") || t.contains("const ") || t.contains("let ")
        || t.contains("=>") || t.contains("export") || t.contains(".js")
    { "javascript" }
    else if t.contains("go ") || t.contains("func ") || t.contains(".go")
    { "go" }
    else if t.contains("int ") || t.contains("void ") || t.contains("printf")
        || t.contains("std::") || t.contains("#include")
    { "cpp" }
    else { "" }
}
...
fn matches_language(query: &str, entry_text: &str) -> bool {
    ...
    if want_rust && lang == "rust" { return true; }
    if want_py && lang == "python" { return true; }
    if want_js && lang == "javascript" { return true; }
    if want_go && lang == "go" { return true; }
    if want_cpp && lang == "cpp" { return true; }
...
impl<const N: usize, const S: usize> TgBot<N, S> {
    fn new(token: String, cube_path: String, mem_path: String, dim: usize) -> Result<Self, String> {
        ...
        println!("Bot: {}^{} dim={}, mem={}, moe={}",
            S, N, cube_dim, ai.memory.size(), moe_total);
    ...
    fn get_updates(&mut self) -> Result<serde_json::Value, String> {
        let url = format!("{}?offset={}&timeout=30&limit=100", self.api_url("getUpdates"), self.offset + 1);
    ...
    fn send_voice(&self, chat_id: i64, text: &str) -> Result<(), String> {
        ...
        body.extend_from_slice(b"--"); body.extend_from_slice(boundary.as_bytes());
    ...
    fn respond(&mut self, query: &str) -> String {
        ...
        if q.contains("как дела") || q.contains("how are you") || q.contains("what's up") {
            self.last_query = q;
            return "Всё отлично! Гипервекторы резонируют, куб активен. Чем могу помочь?".to_string();
        }
        if q.contains("спасибо") || q.contains("thanks") || q.contains("благодарю") {
            self.last_query = q;
            return "Пожалуйста! Обращайся.".to_string();
        }
        if q.contains("пока") || q.contains("bye") || q.contains("goodbye") || q.contains("до свидания") {
            self.last_query = q;
        ...
        if is_binary_question(&q) {
            ...
            if !ctx.is_empty() && !ctx.contains("да или нет") {
                let answer = format!("По твоему предыдущему вопросу «{}» — да. Но если коротко: уточни, что именно тебя интересует.", ctx);
                self.last_query = q;
                return answer;
            }
            self.last_query = q;
        ...
        let tokens: Vec<TokenInfo> = search_query.split_whitespace().map(|w| TokenInfo {
            id: token_id(&w),
            text: w.to_string(),
        }).collect();
        ...
        if !output.super_tokens.is_empty() {
            ...
            if let Some(c) = self.prompts.get("CONCISE") { pv.push(c); }
            if let Some(e) = self.prompts.get("EFFICIENT") { pv.push(e); }
            ...
            for (_, _, entry) in &results {
                ...
                if t.len() < 10 || is_training_artifact(t) { continue; }
                let mut h = std::collections::hash_map::DefaultHasher::new();
                t.hash(&mut h);
                if !seen.insert(h.finish()) { continue; }
        ...
        match cmd {
            ...
            "/code" => {
                ...
                for (_, _, entry) in hits.iter() {
                    ...
                    if raw.len() < 10 || is_training_artifact(raw) { continue; }
                    if !matches_language(&args, raw) { continue; }
                    let mut h = std::collections::hash_map::DefaultHasher::new();
                    raw.hash(&mut h);
                    let hash = h.finish();
                    ...
                    if excerpt.len() > 20 {
                        ...
                        if code_parts.len() >= 4 { break; }
                    }
                }

                let lang = if args.contains("rust") || args.contains("rs") { "rust" }
                    else if args.contains("py") || args.contains("python") { "python" }
                    else if args.contains("js") || args.contains("javascript") { "javascript" }
                    else if args.contains("ts") { "typescript" }
                    else if args.contains("go") { "go" }
                    else if args.contains("c++") || args.contains("cpp") { "cpp" }
                    else if args.contains("c ") || args.contains("cargo") { "c" }
                    else { "rust" };
                ...
                if mode.is_empty() {
                    ...
                    for name in self.prompts.all_modes() {
                        if let Some(hv) = self.prompts.get(&name) {
                            info.push_str(&format!("  {} (dim={}, entropy={:.4})\n", name, hv.dim, hv.entropy()));
                        }
                    }
                            ...
                            Some(ref j) => {
                                ...
                                self.send_message(chat_id, &format!(
                                    "\u{269B} JEPA: dim={}, ctx_len={}\n\
                                     weights: {:?}\n\
                                     loading ratio: {}", j.dim, j.context_len, j.weights, ler))
                            }
                            None => self.send_message(chat_id, "\u{26A0} JEPA не загружен (нет fuga_jepa.bin)")
                        }
                    }
                    "train" => {
                        ...
                        for _ in 0..50 {
                            ...
                            let nearby: Vec<Hypervector> = self.ai.memory.search(&rv, cl + 1)
                        ...
                        if seqs.is_empty() {
                            return self.send_message(chat_id, "Не удалось собрать последовательности");
                        }
                        ...
                        self.send_message(chat_id, &format!(
                            "\u{2705} JEPA обучен: loss={:.4}, sequences={}", loss, seqs.len()))
                    }
            ...
            "/stats" => {
                ...
                let mut info = format!("\u{1F4CA} Записей: {} | Энтропия: {:.4}\n\nДомены:\n", mem, entropy);
                    ...
                    if let Some(results) = updates.get("result").and_then(|v| v.as_array()) {
                        for upd in results {
                            ...
                            if let Some(msg) = upd.get("message") {
                                if let Some(chat) = msg.get("chat").and_then(|c| c.get("id").and_then(|v| v.as_i64())) {
...
fn run_bot<const N: usize, const S: usize>(token: String, cube_path: String, mem_path: String, dim: usize) {

...
fn load_auth_token() -> Option<String> {
    if let Ok(t) = env::var("FUGA_TOKEN") {
        let t = t.trim().to_string();
        if !t.is_empty() {
            return Some(t);
        }
    }
    std::fs::read_to_string("fuga_token")
        .ok()
        .map(|t| t.trim().to_string())
        .filter(|t| !t.is_empty())
}

/// Токен обязателен для всего, кроме статической страницы: без него
/// мутящие POST-эндпоинты и retrieval открыты всей LAN/туннелю.
fn authorized(request: &tiny_http::Request, token: &Option<String>) -> bool {
    match token {
        None => true,
        Some(expected) => request.headers().iter().any(|h| {
            h.field.as_str().as_str().eq_ignore_ascii_case("authorization")
                && h.value.as_str().trim_start_matches("Bearer ").trim() == expected.as_str()
        }),
    }
}

fn read_body_limited(request: &mut tiny_http::Request) -> String {
    let mut buf = Vec::new();
    let _ = request
        .as_reader()
        .take(MAX_BODY_BYTES)
...
fn run_microwave(mw_path: &str, mode: &str, code: &str) -> String {
    // Уникальное имя temp-файла: фиксированный /tmp/mw_eval.rs позволял
    // локальному пользователю через symlink перезаписать цель (аудит 22.08).
...
fn handle_code_fix<const N: usize, const S: usize>(
    ...
    for p in &proposals {
        if let (Some(start), Some(end)) = (p.start_byte, p.end_byte) {
            // replace_range паникует на start > end и на смещении посреди
            // многобайтового символа; паника в хендлере отравляет Mutex
            // состояния и навсегда валит сервер. Небезопасный диапазон —
            // fallback на строкочную замену.
            let safe_range = start <= end
                && end <= fixed.len()
                && fixed.is_char_boundary(start)
                && fixed.is_char_boundary(end);
            if safe_range {
...
fn run_server<const N: usize, const S: usize>(cube_path: &str, port: u16) {
    ...
    let bind_host = env::var("FUGA_BIND").unwrap_or_else(|_| "127.0.0.1".to_string());
    let server = Server::http(&format!("{}:{}", bind_host, port))
        .expect(&format!("Failed to bind port {}", port));
    let auth_token = load_auth_token();
    match &auth_token {
        Some(_) => println!("  🔐 auth: FUGA_TOKEN/fuga_token включён (api/mutations защищены)"),
        None => println!(
            "  ⚠️  auth: токен не задан — API открыт; задайте FUGA_TOKEN или fuga_token"
        ),
    }
    if bind_host == "127.0.0.1" {
        println!("  🔒 bind: {}:{} (LAN/туннель недоступны; FUGA_BIND=0.0.0.0 чтобы открыть)", bind_host, port);
    }
    ...
    for mut request in server.incoming_requests() {
        ...
        if (method != "GET" || url.starts_with("/api")) && !authorized(&request, &auth_token) {
        ...
        let response = match (method.as_str(), url.as_str()) {
            ...
            ("POST", "/api/readout") => {
                let body = read_body_limited(&mut request);

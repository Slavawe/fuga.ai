...
fn main() {
    ...
    println!(
        "  VICReg hybrid_latent λ={} μ={} ν={} γ={:.4} (анти-коллапс, N={})",
        core.vicreg_cfg.lambda,
        core.vicreg_cfg.mu,
        core.vicreg_cfg.nu,
        core.vicreg_cfg.gamma,
        fuga::ai::vicreg::VICREG_BATCH
    );
        ...
        for line in reader.lines() {
            ...
            for i in 0..data.len().saturating_sub(1) {
                ...
                if ckpt_every > 0 && applied >= next_ckpt {
                    ...
                    if n > 0 {
                        let v = core.last_vicreg;
                        println!("\n[ckpt] {} пар: ||t-Wx||_ср={:.4} (||t||={:.4}) consolid={} vicreg_steps={}",
                                 applied, me / n as f32, mt / n as f32, consolidations, core.vicreg_steps);
                        println!("       VICReg total={:.4} inv={:.4} var_hat={:.4} cov_hat={:.4} mean_std={:.4}",
                                 v.total, v.inv, v.var_hat, v.cov_hat, v.mean_std_hat);
                    }
                }

...
impl SecondLegion {
    ...
    pub fn synthesize(&mut self, input: &str, mirror: &mut SelfMirror) -> LegionReport {
        ...
        let hv = crate::ai::temporal_predictor::sdr_to_hypervector(
            &sdr, mirror.predictor.hjepa.dim);
        ...
        let hypothesis = if mirror.predictor.buffer.len() >= mirror.predictor.hjepa.levels[0].context_len {
            ...
            for (tok, tsdr) in &mirror.token_vocab {
                ...
                if o > best_o { best_o = o; best = tok.clone(); }
            }
            best
        } else {
        ...
        let stability = if tm_match > 0.5 { 1.0 - tm_match } else { tm_match };
    ...
    pub fn reflect(&self, mirror: &SelfMirror) -> String {
        format!("[{}] nodes={} token_vocab={} tm_cells={} hyp_history={}",
            self.name, mirror.nodes.len(), mirror.token_vocab.len(),
            mirror.predictor.tm.cells.len(), self.hypothesis_history.len())
    }
}
...
impl FirstLegion {
    ...
    pub fn execute_code(&mut self, code: &str, file_name: &str) -> LegionReport {
        ...
        let anomaly = if outcome.anomaly_triggered {
            ...
            Some(AnomalyEvent::new(pred_count, power,
                if outcome.violations.is_empty() { "None".to_string() }
                else { format!("{:?}", outcome.violations[0]) }
            ))
        } else {
            None
        };

        let stability = if outcome.compiles && outcome.runs { 1.0 }
        else if outcome.compiles { 0.5 }
        else { 0.1 };
        ...
        LegionReport {
            ...
            output: format!("compiles={} runs={} violations={} time={}us stderr={}",
                outcome.compiles, outcome.runs, outcome.violations.len(),
    ...
    pub fn reflect(&self) -> String {
        let ok = self.execution_history.iter().filter(|o| o.compiles && o.runs).count();
        let total = self.execution_history.len();
        format!("[{}] executions={} success={} rate={:.1}%",
            self.name, total, ok,
...
impl LegionCoordinator {
    ...
    pub fn run_cycle(&mut self, input: &str, mirror: &mut SelfMirror) -> Vec<LegionReport> {
        ...
        if hyp_sdr.is_some() {
            let seed = format!("fn auto_generated_{}() {{", self.second.hypothesis_history.len());
            let code = format!("{}\n    println!(\"hypothesis_ok\");\n}}\nfn main() {{ {}(); }}",
                seed, "auto_generated_".to_string() + &self.second.hypothesis_history.len().to_string());
        ...
        if self.reports.len() > 200 { self.reports.drain(0..50); }
        cycle
    }

    pub fn latest_stability(&self) -> f64 {
        self.reports.iter().rev().take(5)
            .map(|r| r.phase_stability)
            .sum::<f64>() / 5.0f64.max(1.0)
    }

    pub fn summary(&self) -> String {
        let last = self.reports.last().map(|r| &r.output).unwrap_or(&"".to_string()).clone();
        format!("[LegionCoord] stability={:.3} reports={} last={}",
            self.latest_stability(), self.reports.len(),

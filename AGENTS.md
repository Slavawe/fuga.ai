...
## Статус обучения (09.08, идёт)
- **Сейчас**: `hybrid_train` (2M шагов, lr-w 0.05, lr-kan 0.3) на 4 корпусах → `fuga_hybrid_2m.fuga` (FUGA1 + KAN_C). Старт 14:43, оценка ~2.6–3 ч на CPU (KAN ≈ 214 steps/s). Корпуса: fisig, doc_code_pairs (крупнейший), training_stack, corpus.
- Уроки скорости: KAN — самый тяжёлый оператор (1.5M коэффициентов); полный прогон на ноутбуке — часы; малые A/B — минуты.

## Реестр инструментов (src/bin/*.rs) — что реально полезно и когда
| Инструмент | Что делает | Когда использовать |
|---|---|---|
| `hybrid_train.rs` | Гибрид W+KAN обучение → FUGA1+KAN_C | **ГЛАВНЫЙ прогон обучения** (сейчас работает) |
| `gpu_train.rs` | Двухскоростной CPU/GPU конвейер (local+patch W на GPU, cap-шейдеры) | Крупные прогоны с GPU; multi-corpus `--jsonl a,b,c` |
...
1. **Оптимизация GPU-KAN**: батчан-диспатч (цикл по парам внутри шейдера, один poll) — устранить launch-overhead; крупные storage-буферы.
2. **Полный прогон 20M+ с гибридом** (W+KAN+OWM) — проверка генерации на большом бюджете.
3. Калибровка KAN (мягкий per-node cap, выше lr) на реальном корпусе.
4. `autonomous_mind` / `mentalese` / `soft_sdr` — редкое использование (0–1 внешних мест): либо интегрировать, либо убрать из mod.rs.
5. Ревизия легаси-модулей src/ (spatial/render/sim/physics; зависимости rapier3d/minifb/hound) — используются ли fuga-web/tgbot-ом.
6. Судьба больших корпусов и чекпоинтов: держать в корне только то, что читает код (см. ниже).

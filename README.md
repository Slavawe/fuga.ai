...
> **Статус: БЕТА-ТЕСТ C++** — ядро обучения и декодеров переведено на C++
> (`cpp/`), оркестрация/CLI/агент — на Python (`py/`), Rust-маск остаётся
> прокси. Формат чекпоинтов (FBW1) bin-совместим с Rust. См. [`cpp/README.md`](cpp/README.md).
...
### Энтропийный патчер (BLT-стиль) — реализовано
`tm_generate_two_speed_entropy` (tm_generate.rs): динамические границы патча по предсказуемости вместо фиксированного размера. Локальный байтовый W даёт cosine-распределение по 256 байтам; **gap топ-1−топ-2** решает: большой gap = предсказуемый участок → локальная эмиссия (патч растёт), малый = сложный → глобальный W выбирает патч. Проектный шаг: gap вместо raw softmax-энтропии (та сатурирует ~0.999 из-за 256 кандидатов с cosine-базлайном ~0.12). A/B 800 .rs: **fixed=50 байт → entropy=200 байт (полный бюджет, 891 B/s)**.

### Спекулятивный draft→verify — реализовано (честный отрицательный)
`tm_generate_speculative`: глобальный W набрасывает патч-черновик, локальный W верифицирует каждый байт. В последовательной VSA-схеме verify не бесплатен (256-кандидный per-byte верификатор дороже выгоды draft): 800-слайс даёт 10 б/11 B/s против entropy 200 б/891 B/s. BLT остаётся лучшим декодером; speculative полезен где параллели не дорог.

### Beam search — реализовано (seed-эхо)
`tm_generate_beam` (beam-3, top-5): классический луч с накопленным log-score вместо greedy top-1. При softmax-uniform по 256 байтам log-prob короткие — луч падает после 1 шага (13 б = seed + 1). Быстрее speculative (491 vs 11 B/s), но контента не добавляет. Ряд всех 4 декодеров (fixed/BLT/speculative/beam) доказал: предел генерации — **не декодер, а локальный байтовый W** (частотная би-паттерна e→r).
